...
Loop1:
  ...
  Loop:
      ...
      continue:
        ...
        ld R9, 0(R6)  # R9 στοιχείο πίνακα Α
        daddi R10, R10, 8 #we don't need dmul because we can keep track of total loops...
        bne R8, R9, elseStatement1 #if A[i][j]==B[i][j]...
        ####Raw stall στο R9###
        #beq R8, R9, ifStatement1 #if A[i][j]==B[i][j]...
        #nop
        #j elseStatement1 #else...
        #nop
        #return:
        ###################################################################
            ifStatement1: # A[i][j]==B[i][j]
              #We will try find the position final[i-1][j-1]
              #R10 contains position of final[i][j]
              #R11 contains position of final[i-1][j-1]
              dsub R11, R10, R16 # we first build the position of final[i-1][j]
        ...
        ########################################################################
            elseStatement1: # A[i][j]!=B[i][j]
              ...
              slt R28, R0, R21 #if 0 < x[i][j-1] - 1
              ld R22, 0(R25)
              beq R28, R29, L11 # go to address of L1
              dsub R22, R22, R29 #R25 = X[i-1][j] - 1
              j L31 #else go to address of L3
              ##
              nop

              L11:
              slt R28, R21, R22 #if x[i][j-1] - 1 < x[i-1][j] - 1
              beq R28, R29, L21 # go to address of L2
              ##
              daddi R4, R4, 8 #j++
              j L41 # else go to address of L4
              nop
              ...
              ##
              daddi R4, R4, 8 #j++
              j L51 # else go to address of L5
              nop
